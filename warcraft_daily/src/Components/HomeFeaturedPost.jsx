...
const HomeFeaturedPost = () => {
  ...
  function getRandomPostNum(postnumber) {
    Math.floor(Math.random(data.length)) * (max - min + 1) + min;
    setRandomPost(getRandomPostNum);
  }

  getRandomPostNum(data.length);

  return (
    <div className="featured-post-wrapper">
      <p className="featured-post-header">Featured Post</p>
      <div className="featured-post-inner">
        <img src={data[1].image} className="post-image"></img>
